...
class BackgroundSubtract:
    ...
    def rolling_ball_background(self, img, radius, light_background=True,
        ...
        background = float_img.astype(img.dtype).reshape((self.height, self.width))

        offset = MAX + 0.5 if invert else 0.5
        for p in range(0, self.width*self.height):
            value = _img[p] - float_img[p] + offset
            value = max((value, 0))
            value = min((value, MAX))
